...
import { UserAvatar, BotAvatar } from "@/components/ui/avatar";
...
const CodePage = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const userMessage: ChatCompletionRequestMessage = {
        role: "user",
        content: values.prompt, // Fixed typo: values.promt should be values.prompt
      };
  ...
  return (
    <div>
      <Heading
        title="Conversation"
        description="Generate text using AI" // Removed unnecessary curly braces
        icon={MessageSquare}
        ...
        <div>
          <div className="space-y-4 mt-4">
            ...
            <div className="flex flex-col-reverse gap-y-4">
              {messages.map(
                (
                  message // Added 'index' as a key to avoid React warnings
                ) => (
                  <div
                    key={message.content}
                    className={cn(
                      "p-8 w-full flex items-start gap-x-8 round -lg",
                      message.role === "user"
